    ...
    while 1
        for j = 1:size(OF.dataMet,1)
            ...
            for k = 1:sizeAB(2)
                ...
                OF.dataMet{j,4}(k) = allMat_sum;%these are total conc and fractions estimated
                OF.dataMet{j,6}(:,k) = allMat_fract;%these are total conc and fractions estimated
            end
        end
        ...
        if ~any(isnan(dataMetMID_EXPvect))%checking
            break
        end
    end
